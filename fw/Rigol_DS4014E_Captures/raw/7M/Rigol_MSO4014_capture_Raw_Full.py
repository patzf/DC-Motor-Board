...
def acquire_raw_channel(ch):
    ...
    try:
        ...
        if points!=7000000:
            raise RuntimeError(f"CH{ch} acquisition memory is {points} samples instead of 7000000")
        tcp_send(sock,":WAVeform:POINts 7000000")
        points_set=int(tcp_query(sock,":WAVeform:POINts?"))
        print(f"CH{ch} POINTS SET:",points_set)
        if points_set!=7000000:
            raise RuntimeError(f"CH{ch} scope did not accept 7000000 points; returned {points_set}")
        ...
        print(f"CH{ch} EXPECTED POINTS:",7000000)
        if total!=7000000:
            raise RuntimeError(f"CH{ch} received {total} samples instead of 7000000")
        voltages=[(raw_value-y_ref-y_orig)*y_inc for raw_value in raw_data]
        current_time_axis=[x_orig+(idx*x_inc) for idx in range(7000000)]
...
try:
    ...
    if not validated_channels:
        ...
        exit()
    print(f"Capturing FULL 7M RAW data for channels: {validated_channels}")
    for ch in validated_channels:
        print(f"\nStarting FULL 7M RAW acquisition for CH{ch}...")
